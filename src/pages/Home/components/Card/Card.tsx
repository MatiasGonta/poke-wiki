...
const Card = ({ name, url }: RawPokemon) => {
    ...
    return (
        ...
        >
            <h2 className="font-bold text-poke-primary text-[25px] pb-5 border-b border-b-poke-muted min-[450px]:pb-2">{capitalizeFirstLetterOfString(name)}</h2>
            {
                isLoading
                    ? <CardSkeleton />
                    : !error && pokemonData
...
const CardSkeleton = () => {
    ...
    return (
        <div className="w-full h-full overflow-hidden rounded-md flex flex-col justify-between">
